...
pair<bool, float> is_on_line_optimized(
    ...
    const auto& new_pixel = pixels[new_pixel_index].value;
...
auto search_on_the_line(
    ...
    return search_on_the_line_optimized(pixels, lines, line_distance, 
                                       current_pixel, threshold, calculate_from_current);
}
...
//         }
//     } else {
//         auto is_on = is_on_line(pixels, lines.back(), line_distance.back(), current_pixel, threshold);
...
//         lines[minimum_distance.first].push_back(current_pixel);
//         line_distance[minimum_distance.first].push_back(minimum_distance.second);
...
//         }
//     } else {
//         auto can_be = can_be_on_line(pixels, lines.back(), line_distance.back(), current_pixel, threshold);
...
//         lines[minimum_distance.first].push_back(current_pixel);
//         for (size_t i = 0; i < line_distance[minimum_distance.first].size(); i++) {
//             line_distance[minimum_distance.first][i] = minimum_distance.second.second[i];
//         }
//         line_distance[minimum_distance.first].push_back(minimum_distance.second.first);
...
//             search_result = search_on_the_line(pixels, lines, lines_distances, current_pixel, threshold, true);
//         } 

//         // If not added to last line, search all lines
//         if (search_result == -1 && !lines.empty()) {
//             search_result = search_on_the_line(pixels, lines, lines_distances, current_pixel, threshold, false);
//             if (search_result != -1) {
